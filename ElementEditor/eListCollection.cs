...
namespace NpcGen_Editor.sELedit
{
    public class eListCollection
    {
        ...
        public string GetItemName(int id, string listName = "_ESSENCE")
        {
            for (int i = 0; i < Lists.Length; i++)
            {
                if (i != ConversationListIndex)
                {
                    if (!Lists[i].listName.Contains(listName))
                        continue;

                    int pos = -1;
                    for (int j = 0; j < Lists[i].elementFields.Count(); j++)
                    {
                        if (Lists[i].elementFields[j] == "Name")
                        {
                            pos = j;
                            break;
                        }
                    ...
                    for (int e = 0; e < Lists[i].elementValues.Count(); e++)
                    {
                        if (Lists[i].elementFields[0] == "ID")
                        {
                            if (GetValue(i, e, 0) == id.ToString())
                            {
                                return GetValue(i, e, pos);
                            }
                        }
                        else
                        {
        ...
        {
            if (!File.Exists(elFile)) return null;
